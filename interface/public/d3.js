!(function () {
        ...
        for (var j = -1, m = this.length; ++j < m; ) {
            for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
                if ((node = group[i])) {
                    subgroups.push(
                        (subgroup = d3_array(
                            selector.call(node, node.__data__, i, j)
                        ))
                    );
            ...
            if (key) {
                ...
                for (i = -1; ++i < n; ) {
                    if ((node = group[i])) {
                        if (
                            nodeByKeyValue.has(
                                (keyValue = key.call(node, node.__data__, i))
                            )
                        ) {
                            exitNodes[i] = node;
                        } else {
                ...
                for (i = -1; ++i < n; ) {
                    if (
                        i in keyValues &&
                        nodeByKeyValue.get(keyValues[i]) !== true
                    ) {
                        exitNodes[i] = group[i];
                    }
                }
        ...
        if (d2 < ε2) {
            ...
            i = function (t) {
                var s = t * S,
                    ...
                    u =
                        (w0 / (ρ2 * d1)) *
                        (coshr0 * d3_tanh(ρ * s + r0) - d3_sinh(r0));
                return [
                    ux0 + u * dx,
                    uy0 + u * dy,
    ...
    function d3_locale_timeFormat(locale) {
            ...
            function format(date) {
                ...
                while (++i < n) {
                    if (template.charCodeAt(i) === 37) {
                        ...
                        if (
                            (p = d3_time_formatPads[(c = template.charAt(++i))]) !=
                            null
                        )
                            c = template.charAt(++i);
        ...
        function d3_time_parse(date, template, string, j) {
            ...
            while (i < n) {
                ...
                if (c === 37) {
                    ...
                    p =
                        d3_time_parsers[
                            t in d3_time_formatPads ? template.charAt(i++) : t
                        ];
        ...
        function d3_time_parseAmPm(date, string, i) {
            var n = d3_time_periodLookup.get(
                string.slice(i, (i += 2)).toLowerCase()
            );
        ...
        for (var i = 0, n = polygon.length; i < n; ++i) {
            ...
            while (true) {
                ...
                if (antimeridian ^ (λ0 >= meridian) ^ (λ >= meridian)) {
                    ...
                    var φarc =
                        (antimeridian ^ (dλ >= 0) ? -1 : 1) *
                        d3_asin(intersection[2]);
                    if (
                        parallel > φarc ||
                        (parallel === φarc && (arc[0] || arc[1]))
                    ) {
        ...
        return function (listener) {
            ...
            function linePoint(x, y) {
                x = Math.max(
                    ...
                    Math.min(d3_geo_clipExtentMAX, x)
                );
                y = Math.max(
                    -d3_geo_clipExtentMAX,
                    Math.min(d3_geo_clipExtentMAX, y)
                );
    ...
    function d3_geo_interpolate(x0, y0, x1, y1) {
        var cy0 = Math.cos(y0),
            ...
            d =
                ...
                Math.asin(
                    Math.sqrt(
                        d3_haversin(y1 - y0) + cy0 * cy1 * d3_haversin(x1 - x0)
                    )
                ),
            k = 1 / Math.sin(d);
        var interpolate = d
    ...
    function d3_geom_hullUpper(points) {
        ...
        for (var i = 2; i < n; i++) {
            while (
                hs > 1 &&
                d3_cross2d(points[hull[hs - 2]], points[hull[hs - 1]], points[i]) <=
                    0
            )
                --hs;
            hull[hs++] = i;
    ...
    function d3_interpolateScale(ka, kb, s, q) {
        if (ka[0] !== kb[0] || ka[1] !== kb[1]) {
            var i = s.push(
                d3_interpolateTransformPop(s) + 'scale(',
                null,
                ',',
                null,
                ')'
            ...
            if (padding) {
                var dr =
                    (padding *
                        (radius
                            ? 1
                            : Math.max((2 * root.r) / w, (2 * root.r) / h))) /
                    2;
                d3_layout_hierarchyVisitAfter(root, function (d) {
                    d.r += dr;
                });
    ...
    function d3_scale_linearTickFormat(domain, m, format) {
        ...
        if (format) {
            ...
            if (match[8] === 's') {
                ...
                if (!match[7])
                    match[7] =
                        '.' + d3_scale_linearPrecision(prefix.scale(range[2]));
                match[8] = 'f';
            ...
            ? Math.abs(
                  p -
                      d3_scale_linearPrecision(
                          Math.max(abs(range[0]), abs(range[1]))
                      )
              ) + +(type !== 'e')
            : p - (type === '%') * 2;
    }
    ...
    function d3_scale_ordinal(domain, ranger) {
        ...
        scale.rangeRoundBands = function (x, padding, outerPadding) {
            ...
            range = steps(
                start +
                    Math.round(
                        (stop - start - (domain.length - padding) * step) / 2
                    ),
                step
            );
            if (reverse) range.reverse();
    ...
    d3.svg.arc = function () {
        ...
        function arc() {
            ...
            if (
                ...
                if (x1 != null) {
                    var t30 = d3_svg_arcCornerTangents(
                            x3 == null ? [x2, y2] : [x3, y3],
                            [x0, y0],
                            r1,
                            rc1,
    ...
    d3_transitionPrototype.styleTween = function (name, tween, priority) {
        ...
        function styleTween(d, i) {
            var f = tween.call(
                ...
                d,
                i,
                d3_window(this).getComputedStyle(this, null).getPropertyValue(name)
            );
            return (
                f &&
                function (t) {
    ...
    function d3_time_scale(linear, methods, format) {
        ...
        function tickMethod(extent, count) {
            ...
            return i == d3_time_scaleSteps.length
                ...
                : methods[
                      target / d3_time_scaleSteps[i - 1] <
                      d3_time_scaleSteps[i] / target
                          ? i - 1
                          : i
                  ];
        }
